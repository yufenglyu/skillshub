...
- **技能集合** 用于把中央技能组成可复用分组，并批量安装到目标位置。
...
- 设置页的项目目录管理位于软件平台下方，项目目录作为项目级安装目标使用 `.agents/skills`。
...
推送 `v0.16.0` 这样的版本 tag 后，GitHub Actions 会构建并发布桌面安装包。发布工作流会从 `CHANGELOG.md` 读取对应版本的 release notes，因此每次发布都必须有匹配的更新日志条目。
