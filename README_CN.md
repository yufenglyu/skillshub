...
| **项目目录** | 项目级安装目标 | `<项目>/.agents/skills` |
| **技能集合** | 把资源库技能组成可复用分组 | 应用数据库 |

应用数据保存在 `~/.skillshub/db.sqlite`。从旧版本升级时，如果新数据库还不存在，首次启动会迁移已有的 `~/.skillsmanage/db.sqlite`。
...
- 设置页支持编辑内置平台、添加自定义平台、管理项目目录、本地 ZIP / WebDAV 备份，以及检查更新。备份不会包含 API Key、Token 或密码类内容。
...
左侧栏默认只展示本机已存在对应 skills 目录的内置平台；也可以手动切换为显示全部平台。软件平台分组和项目目录列表都可以独立折叠。
...
SkillsHub 支持导出和导入完整本地备份。WebDAV 备份支持测试连接、查看远端列表、上传、选择恢复和删除选中备份。
...
推送 `v0.30.0` 这样的版本 tag 后，GitHub Actions 会构建并发布桌面安装包。发布工作流会从 `CHANGELOG.md` 读取对应版本的 release notes，因此每次发布都必须有匹配的更新日志条目。
