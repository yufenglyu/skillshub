...
*截图来自当前 v1.1.1 代码的界面，使用虚构仓库、技能和路径演示；不代表预装内容。*
...
点击工具栏“更新技能”后，可选择 **上次结果 / 检查更新 / 更新星标数 / 取消**。检查更新需确认后开始；更新星标数仅获取仓库统计，不下载技能或覆盖上次检查结果。

![更新技能：检查更新或单独刷新星标数](docs/images/zh/update-actions.jpg)

状态栏“更新状态”显示检查时间，点击可查看结果：按仓库折叠浏览，按状态筛选，展开查看文件增删改。
...
- 新旧技能替换在“远程删除”中选择对应项，勾选后使用底部“删除并重导”；无法确定对应关系时需手动配对。
- 弹窗可调整大小，切换筛选不会改变高度。
- 导入、检查更新、应用更新和 AI 生成进入后台任务队列，可查看进度、结果，停止任务、重试失败任务或手动清理已结束任务。正在执行的任务会在当前步骤完成后停止。
...
- 搜索框内的筛选按钮可选择仓库名称、技能名称、描述和备注范围；目录也可以记录备注。筛选弹窗在焦点移出、点击外部或按 Esc 时关闭。
- 右侧预览支持概览、文档和安装信息；表格支持列显隐、排序、拖动列顺序和调整列宽。名称标题右侧可切换 `作者/仓库` 与 `仓库@作者`，名称排序跟随显示格式。
- 技能备注和标签使用 **AI / 保存 / 清空**：AI 每次生成可编辑建议；标签建议追加并去重，保留已有标签，保存后才写入。AI 服务与提示词可自行配置。
...
本地和 WebDAV 仓库备份均保留已缓存的 GitHub 星标数及检查时间；导出的 CSV 仓库清单也包含星标数。
