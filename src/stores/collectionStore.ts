...
export async function getCollectionDetails(ids: string[]): Promise<CollectionDetail[]> {
  if (!isTauriRuntime()) return ids.includes(BROWSER_FIXTURE_COLLECTION_DETAIL.id) ? [BROWSER_FIXTURE_COLLECTION_DETAIL] : [];
  return Promise.all(ids.map(collectionId => invoke<CollectionDetail>("get_collection_detail", {collectionId})));
}
...
export const useCollectionStore = create<CollectionState>((set, get) => ({
  ...
  loadCollectionDetail: async (id: string) => {
    ...
    try {
      ...
      if (request === detailRequest) set({ error: String(err), isLoadingDetail: false });
    }
  },
