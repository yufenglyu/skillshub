...
describe("InstallDialog", () => {
  ...
  it("hides software platforms that are not detected", async () => {
    render(
      <InstallDialog
        open
        onOpenChange={vi.fn()}
        ...
        agents={[
          ...agents,
          {
            ...
            global_skills_dir: "~/.missing/skills",
            is_detected: false,
            is_builtin: true,
            is_enabled: true,
          },
    ...
    const dialog = await screen.findByRole("dialog", { name: /安装 api-design|Install api-design/i });
    expect(within(dialog).queryByLabelText("Missing Platform")).not.toBeInTheDocument();
    expect(within(dialog).getByLabelText("Cursor")).toBeInTheDocument();
    expect(within(dialog).getByLabelText("temp")).toBeInTheDocument();
  });
