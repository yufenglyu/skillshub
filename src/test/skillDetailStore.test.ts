...
describe("tree detail request ordering", () => {
  beforeEach(() => { vi.clearAllMocks(); useSkillDetailStore.getState().reset(); });
  it("ignores a late response for the previously selected skill", async () => {
    let finishOld!: (detail: SkillDetail) => void;
    vi.mocked(invoke).mockImplementation(async (command, args) => {
      if (command === "get_skill_detail") {
        if ((args as {skillId:string}).skillId === "old") return await new Promise<SkillDetail>(resolve => {finishOld=resolve;});
        return {...mockDetail,id:"new",file_path:"/new/SKILL.md"};
      }
      return "new content";
    });
    const old = useSkillDetailStore.getState().loadDetail("old");
    await useSkillDetailStore.getState().loadDetail("new");
    finishOld({...mockDetail,id:"old"}); await old;
    expect(useSkillDetailStore.getState().detail?.id).toBe("new");
    expect(useSkillDetailStore.getState().content).toBe("new content");
  });
  it("does not restore detail after its inspector has unmounted", async () => {
    let finish!: (detail: SkillDetail) => void;
    vi.mocked(invoke).mockImplementation(async () => await new Promise<SkillDetail>(resolve => {finish=resolve;}));
    const request=useSkillDetailStore.getState().loadDetail("old");
    useSkillDetailStore.getState().reset();finish(mockDetail);await request;
    expect(useSkillDetailStore.getState().detail).toBeNull();
  });
});
