...
describe("skill browser preferences", () => {
  ...
  it("persists skill table columns", () => {
    ...
    expect(result.current.visibleColumns.has("createdAt")).toBe(true);

    act(() => result.current.toggleColumn("createdAt"));
    rerender();

    expect(result.current.visibleColumns.has("createdAt")).toBe(false);
  });
  ...
  it("does not toggle fixed skill columns", () => {
    ...
    expect(result.current.visibleColumns.has("actions")).toBe(false);
  });
});
