...
pub async fn set_setting_impl(pool: &DbPool, key: &str, value: &str) -> Result<(), String> {
    ...
    if key == "ai_note_prompt" && db::get_setting(pool, key).await?.as_deref() != Some(value) {
        sqlx::query("DELETE FROM skill_explanations").execute(pool).await.map_err(|e| e.to_string())?;
    }
...
pub async fn update_central_skills_dir_impl(pool: &DbPool, path: &str) -> Result<Agent, String> {
    ...
    demote_skills_outside_new_central_root(pool, &old_central.global_skills_dir, &expanded_path)
        .await?;
    db::upsert_builtin_scan_directory(pool, &expanded_path, "Central Skills").await?;
...
mod tests {
    ...
    #[tokio::test]
    async fn note_prompt_changes_clear_cached_explanations() {
        let pool = setup_test_db().await;
        set_setting_impl(&pool, "ai_note_prompt", "old").await.unwrap();
        sqlx::query("INSERT INTO skill_explanations (skill_id, lang, model, explanation) VALUES ('test', 'zh', 'model', 'cached')")
            .execute(&pool).await.unwrap();
        set_setting_impl(&pool, "ai_note_prompt", "old").await.unwrap();
        let count: i64 = sqlx::query_scalar("SELECT COUNT(*) FROM skill_explanations").fetch_one(&pool).await.unwrap();
        assert_eq!(count, 1);
        set_setting_impl(&pool, "ai_note_prompt", "new").await.unwrap();
        let count: i64 = sqlx::query_scalar("SELECT COUNT(*) FROM skill_explanations").fetch_one(&pool).await.unwrap();
        assert_eq!(count, 0);
        assert_eq!(get_setting_impl(&pool, "ai_note_prompt").await.unwrap().as_deref(), Some("new"));
    }
